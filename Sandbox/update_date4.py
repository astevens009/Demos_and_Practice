...
def set_current_date(file_path):
    ...
    with open(file_path, mode='r') as edit_file:
        ...
        for index, line in enumerate(items):
            ...
            if copyright_match:
                print("Target found at line index {}".format(index))
...
def backup_file(source_file):
    ...
    file_info = os.path.basename(source_file).split('.')
    
    file_name = file_info[0]
    file_ext = file_info[1]

    os.makedirs("Sandbox\\Backup", exist_ok=True)
    shutil.copyfile(source_file, "Sandbox\\Backup\\bak_{}_{}.{}}".format(file_name, version, file_ext))
